import { CircuitInfo, Direction, Position, format, log, mouseX, mouseY, scrX, scrY, zoomLevel } from "./globals.js";
import { Player } from "./player.js";

export class Map {
  static tiles: Tile[] = [];
  static tileAt(pos: Position) {
...
export class Tile {
  ...
  isConnected(other: Tile) {
    if((other.pos.plus(0, 1).equals(this.pos) && other.hasConnection(Direction.down)) ||
      ...
      (other.pos.plus(-1).equals(this.pos) && other.hasConnection(Direction.left))) {return true;}
    else return false;
  }
  tick() {}
}
...
function renderWire(context: CanvasRenderingContext2D, wire: WireTile, color?: string) {
  ...
  if(u && (u instanceof WireTile || wire.isConnected(u))) context.fillRect(left + zoomLevel * 2/5, top, zoomLevel * 1/5, zoomLevel * 2/5);
  let d = Map.tileAt(wire.pos.plus(0, 1));
  if(d && (d instanceof WireTile || wire.isConnected(d))) context.fillRect(left + zoomLevel * 2/5, top + zoomLevel * 3/5, zoomLevel * 1/5, zoomLevel * 2/5);
  let l = Map.tileAt(wire.pos.plus(-1));
  if(l && (l instanceof WireTile || wire.isConnected(l))) context.fillRect(left, top + zoomLevel * 2/5, zoomLevel * 2/5, zoomLevel * 1/5);
  let r = Map.tileAt(wire.pos.plus(1));
  if(r && (r instanceof WireTile || wire.isConnected(r))) context.fillRect(left + zoomLevel * 3/5, top + zoomLevel * 2/5, zoomLevel * 2/5, zoomLevel * 1/5);
}
